...
stellen sie dar. Rechnen: ``selection`` / ``ts`` / ``pyod_engine``.
"""
...
from . import config
from .ts import iter_run_sequences

# --- Stil -----------------------------------------------------------------------------
FIGSIZE = (10.0, 5.6)  # 16:9
DETECTOR_COLORS = {"ecod": "#4C72B0", "iforest": "#55A868", "ocsvm": "#C44E52", "pca": "#8172B3"}
...
def _one_fault_run(split, fault: int):
    ...
    faultno = split.meta_test["faultNumber"].to_numpy()
    for rows, _ in iter_run_sequences(split.X_test, split.meta_test):
        if faultno[rows][0] == fault:
            return rows, split.meta_test["sample"].to_numpy()[rows]
...
# --- 2) Fenster-Mechanismus (Schlüsselgrafik) -----------------------------------------
def fig_windowing_mechanism(split, scores: np.ndarray, *, fault: int, window: int, step: int,
                            sensor: int = 0, save_as: str | None = None):
    rows, sample = _one_fault_run(split, fault)
    signal = split.X_test[rows, sensor]
    score = _norm01(np.asarray(scores)[rows])

    fig, (axt, axb) = plt.subplots(2, 1, sharex=True, figsize=(10.0, 5.8))
    axt.plot(sample, signal, color="#34495E", lw=1.3)
    axt.set(ylabel=f"Sensor {sensor + 1}\n(skaliert)")
    axt.set_title(f"Schritt 1–2: gleitende Fenster (w={window}) → Detektor scort jedes Fenster",
                  fontsize=12, fontweight="normal", loc="left", pad=6)
    # ein paar Fenster einzeichnen (Beschriftung INNEN, damit nichts mit dem Titel kollidiert)
    starts = np.linspace(0, len(sample) - window, 3).astype(int)
    ylo, yhi = axt.get_ylim()
    for i, s0 in enumerate(starts):
        x0, x1 = sample[s0], sample[min(s0 + window - 1, len(sample) - 1)]
        axt.add_patch(Rectangle((x0, ylo), x1 - x0, yhi - ylo, color=HIGHLIGHT, alpha=0.20, zorder=0))
        axt.text((x0 + x1) / 2, yhi, f"F{i + 1}", ha="center", va="top",
                 fontsize=10.5, color=HIGHLIGHT, fontweight="bold")

    axb.plot(sample, score, color=DETECTOR_COLORS["pca"], lw=1.6)
    axb.set(xlabel="Zeit (sample)", ylabel="Anomalie-Score\n(normiert)")
    axb.set_title("Schritt 3: Fenster-Scores → ein Score pro Zeitpunkt (max-Aggregation)",
                  fontsize=12, fontweight="normal", loc="left", pad=6)
    # Auffälliger Pfeil quer über die Panel-Grenze: mittleres Fenster (oben) → sein Score (unten).
    xc = sample[min(starts[1] + window // 2, len(sample) - 1)]
    fig.add_artist(ConnectionPatch(
        xyA=(xc, ylo), coordsA=axt.transData, xyB=(xc, score[starts[1] + window // 2]),
        coordsB=axb.transData, arrowstyle="-|>", mutation_scale=24, color=HIGHLIGHT, lw=2.6,
    ))
    axb.text(xc + 15, 0.82, "jedes Fenster → 1 Score", ha="left", va="center",
             fontsize=11.5, color=HIGHLIGHT, fontweight="bold",
             transform=axb.get_xaxis_transform())
    return finish(fig, title="Time-Series-Awareness: der Fenster-Adapter",
                  takeaway="Aus dem Verlauf wird ein Merkmalsvektor — der Detektor sieht Dynamik statt Einzelpunkte.",
                  save_as=save_as)


# --- 3) i.i.d. vs. zeitbewusst --------------------------------------------------------
def fig_iid_vs_windowed(iid_auc: dict[str, float], tw_auc: dict[str, float], *, save_as: str | None = None):
    names = sorted(tw_auc)
    x = np.arange(len(names))
    w = 0.38
    fig, ax = plt.subplots()
    b1 = ax.bar(x - w / 2, [iid_auc[n] for n in names], w, label="i.i.d. (zeilenweise)", color=MUTED)
    b2 = ax.bar(x + w / 2, [tw_auc[n] for n in names], w, label="zeitbewusst (gefenstert)",
                color=[_color(n) for n in names])
    ax.set(xticks=x, ylim=(0, 1.08), ylabel="ROC-AUC (illustrativ)")
    ax.set_xticklabels(names)
    ax.legend(loc="lower right")
    for n, xi in zip(names, x, strict=True):
        d = tw_auc[n] - iid_auc[n]
        ax.text(xi + w / 2, tw_auc[n] + 0.015, f"{d:+.3f}", ha="center", fontsize=11.5,
                fontweight="bold", color=OK if d >= 0 else WARN)
    for bars in (b1, b2):
        for b in bars:
            ax.text(b.get_x() + b.get_width() / 2, b.get_height() - 0.06, f"{b.get_height():.2f}",
                    ha="center", fontsize=10, color="white", fontweight="bold")
    return finish(fig, title="Zeitbewusst schlägt den i.i.d.-Blick",
                  takeaway="Dieselben Detektoren, aber gefenstert: die zeitliche Struktur zu nutzen hilft messbar (Δ).",
                  caveat=CAVEAT_AUC, save_as=save_as)


# --- 4) Score-Zeitreihe eines Fehlerlaufs ---------------------------------------------
def fig_score_timeline(split, scores: np.ndarray, *, fault: int, onset: int, detector: str = "pca",
    ...
    ax.plot(sample, score, color=_color(detector), lw=1.7, label=f"{detector}-Score (zeitbewusst)")
...
# --- 6b) Differenzierung pro Fehlerart --------------------------------------------------
def fig_per_fault(breakdown: dict[int, dict[str, float]], *, hard_faults: list[int] | None = None,
    ...
    Schwarm-Einigkeit flach hoch (warnt also nicht).
    """
    ...
    axg.set_title("Vertrauenssignal agreement (label-frei) — bleibt auch bei den unsichtbaren Fehlern hoch",
                  fontsize=12, fontweight="normal", loc="left", pad=6)
    axg.set_xticks(x)
...
def fig_final_comparison(strategies: dict[str, tuple[str, float]], oracle_auc: float,
                         *, save_as: str | None = None):
    ...
    ax.axhline(oracle_auc, color=WARN, ls="--", lw=1.8, label=f"Oracle-Obergrenze = {oracle_auc:.3f}")
    ...
    return finish(fig, title="Ohne Labels ≈ Oracle-Obergrenze",
                  takeaway="Label-freie Konsens-Auswahl erreicht praktisch das Optimum — ganz ohne Labels.",
                  caveat=CAVEAT_AUC, save_as=save_as)
